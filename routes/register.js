...
const client = new MongoClient("mongodb+srv://Esthe:P04CdRpefX1mvguy@cluster0.pkbexks.mongodb.net/?retryWrites=true&w=majority")
...
const getHashedPassword = (password) => {
  ...
  router.post("/", (req, res) => {
    ...
    if (
      ...
      checkUser(client, email).then(async (checked) => {
        if (checked) {
          res.render("register", {
            reg: "Este usuário já foi cadastrado",
            user,
          });
        } else {
          ...
          res.render("login", {
            log: "Cadastro completo.Faça o login",
            user,
          });
        }
      });
  ...
  async function checkUser(client, email) {
    try {
      ...
      if (result !== null) {
        console.log("Temos esse email");
        return true;
      } else {
        console.log("Não temos esse email");
        return false;
      }
    } finally {
      await client.close();
